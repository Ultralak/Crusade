...
func generate_dungeon() -> void:
	...
	dungeon_layout[current_pos] = "START"
	...
	while dungeon_layout.size() < max_rooms:
		...
		if not dungeon_layout.has(current_pos):
			dungeon_layout[current_pos] = "COMBAT"
...
func spawn_rooms() -> void:
	...
	for grid_pos in dungeon_layout.keys():
		var selected_scene : PackedScene = room_scenes.pick_random()
		var room_instance = selected_scene.instantiate()
