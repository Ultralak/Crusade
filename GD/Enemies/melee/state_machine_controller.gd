...
func _physics_process(_delta: float) -> void:
	...
	if current_state == "attack" or current_state == "death" or current_state == "recovery" or current_state == "hurt":
		return
	if  distance < attack_distance and in_range(enemy_1.global_position.y , player_pos.y, height_offset):
		if current_state != "attack":
			node_finite_state_machine.transition_to("attack")
	elif distance > attack_distance and distance < detect_distance and in_range(enemy_1.global_position.y , player_pos.y, height_offset):
