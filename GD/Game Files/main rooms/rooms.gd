...
func _close_entrance()->void:
	...
	ground.to_local(right.global_position)
	coords = ground.local_to_map(ground.to_local(right.global_position))
...
func _spawn_enemies(markers : Marker2D) -> void:
	...
	spawn_explosion_instance.position = markers.position
	...
	if is_instance_valid(markers) and is_inside_tree():
		var choice : ENEMYTYPE = ENEMYTYPE.RANGE
		#ENEMY_SCENES.keys().pick_random()
