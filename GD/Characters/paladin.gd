extends Character
#damage and health script

#temp code for weapon will build upon tmr
@onready var sword : Node = get_node("sword")

func _process(_delta: float) -> void:
	var mouse_direction : Vector2 = (get_global_mouse_position() - global_position).normalized()
	
	if mouse_direction.x > 0 and animated_sprite_2d.flip_h:
		animated_sprite_2d.flip_h = false
	elif mouse_direction.x < 0 and not animated_sprite_2d.flip_h:
		animated_sprite_2d.flip_h = true
		
	sword.rotation = mouse_direction.angle()
	#if sword.scale.y == 1 and mouse_direction.x < 0:
		#sword.scale.y = -1
	#elif sword.scale.y == -1 and mouse_direction.x > 0:
		#sword.scale.y = 1
	
func get_input() -> bool:
	#Need to change for 3d world only works for top down like soul knight not wizard of legends
	move_direction = Vector2.ZERO
	var pressed : bool = false
	if Input.is_action_pressed("move_down"):
		move_direction += Vector2.DOWN
		pressed = true
	if Input.is_action_pressed("move_up"):
		move_direction += Vector2.UP
		pressed = true
	if Input.is_action_pressed("move_left"):
		move_direction += Vector2.LEFT
		pressed = true
	if Input.is_action_pressed("move_right"):
		move_direction += Vector2.RIGHT
		pressed = true
	return pressed
#regarding weapons i will need to make the weapon pickable and an object that when interacted can be picked up	

		
		
